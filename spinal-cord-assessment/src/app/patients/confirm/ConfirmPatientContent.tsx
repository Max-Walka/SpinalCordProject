...
async function getNextId(tableName: string, idColumn: string) {
  ...
  if (error) {
    throw new Error(
      `Could not determine next ${idColumn} for ${tableName}: ${error.message}`
    );
  }
  ...
  async function handleRegisterPatient() {
    ...
    try {
      ...
      if (patientNameInsertError) {
        throw new Error(
          `Patient Name insert failed: ${patientNameInsertError.message}`
        );
      }
      ...
      if (patientContactInsertError) {
        throw new Error(
          `Patient Contact insert failed: ${patientContactInsertError.message}`
        );
      }
      ...
      if (patientAddressInsertError) {
        throw new Error(
          `Patient Address insert failed: ${patientAddressInsertError.message}`
        );
      }
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <span>
                I confirm that this patient has consented to their clinical data
                being stored in the ISNCSCI assessment system, in accordance
                with the New Zealand Health Information Privacy Code 2020.
              </span>
            </label>
          </div>
